...
def AFS (order, q = 0.95, d = 0.25):
    ...
    loess_fit = r.loess("y ~ x", data=df, degree = 2, span = d)
    ...
    index=np.sort(index)
    # Have to append the last element for predict function to work
    ...
    loess_fit2 = r.loess("y_2 ~ x_2", data=df2, degree = 2, span = d)
...
np.savetxt("rpy2.csv", result, delimiter=",", fmt='%s')
